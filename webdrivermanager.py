...
class WebDriverManagerBase:
    ...
    def download(self, version='latest', show_progress_bar=True):  # pylint: disable=inconsistent-return-statements
        ...
        download_file_url = os.path.join(download_url, filename)
        ...
        data = requests.get(download_file_url, stream=True)
...
#    chrome_driver_base_url = 'https://www.googleapis.com/storage/v1/b/chromedriver'
    chrome_driver_base_url = 'https://npm.taobao.org/mirrors/chromedriver'
...
#        chrome_driver_objects = requests.get(self.chrome_driver_base_url + '/o').json()
        ...
        # url = "https://cdn.npm.taobao.org/dist/chromedriver/79.0.3945.36/"
        url = self.chrome_driver_base_url + '/{}/'.format(version)
        filename = 'chromedriver_{}{}.zip'.format(self.os_name, local_bitness)
        return (url, filename)
